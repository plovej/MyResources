...
/**
 * <p>
 * Description: Number相关的公共方法类
 * @author zhugl
 */
@SuppressLint("UseValueOf")
public class NumberUtils {

    public final static int DRFAULT_VALUE = -2;
    public final static float DIGIT_DRFAULT_VALUE = -2.0f;
    /**
     * 数字中默认保留的小位数
     */
    public final static int DIGIT_DEFAULT_NUM = 2;

    public final static String DECIMAL_DEFAULT_FORMAT = "###########0.##";
    ...
    /**
     * Description: 将int转化为String
     * @param value
     * @return 转化后的String
     */
    public static String toString(int value) {

        if(value == DRFAULT_VALUE){
            return "";
        }
     ...
     *
     * @param amount     double 待处理的数字
     * @param nCount     int 保留小数的位数
     * @param bDischarge boolean 是否截取整数
     ...
     * Description: 默认保留两位小数的四舍五入方法
     *
     * @param amount BigDecimal 待round的数字
    ...
    public static BigDecimal round(BigDecimal amount, int digits) {
        ...
        return amount.setScale(digits, BigDecimal.ROUND_HALF_UP);
        //return amount.divide(one, digits, RoundingMode.HALF_UP);
//        Double dAmount = Double.valueOf(amount.toString());
//        return new BigDecimal(round(dAmount,digits));
    }

    /**
     * Description: 默认保留两位小数的四舍五入方法
     *
     * @param amount String 待round的数字
     * @return BigDecimal round后的数字
     */
    ...
    /**
     * Description: 对小数进行round(默认保留两位小数),两位小数后的小数全部截取掉. round规则:1-5的round为5,6-9的round为0且进位
     *
     * @param value double 待round的数字
     * @return double round后的数字
     */
    ...
    /**
     * Description: 将指定位数上的小数进行round,指定位数后的小数全部截取掉. round规则:1-5的round为5,6-9的round为0且进位
     *
     * @param value  double 待round的数字
     * @param digits int 指定的小数位置
     * @return double round后的数字
     */
    ...
    /**
     * Description: 对小数进行向上round(默认保留两位小数),两位小数后的小数全部截取掉. round规则:指定位数后的小数不全为0的则进位
     *
     * @param value double 待round的数字
     * @return double round后的数字
     */
    public static double roundUp(double value) {
        if (value == truncate(value, DIGIT_DEFAULT_NUM)) {
            ...
            return (new BigDecimal(toString(truncate(value, DIGIT_DEFAULT_NUM)))).add(new BigDecimal(toString(1.0 / Math.pow(10, DIGIT_DEFAULT_NUM)))).doubleValue();
        }
    }
    ...
    public static double roundUp(double value, int digits) {
        if (value == truncate(value, digits)) {
            ...
            return (new BigDecimal(toString(truncate(value, digits)))).add(new BigDecimal(toString(1.0 / Math.pow(10, digits)))).doubleValue();
        }
    }

    /**
     * Description: 该方法处理的过程如下: 1.对小数进行round(默认保留两位小数) 2.取得到的数字是metavalue的倍数值,并进行进位round 3.进位后的倍数值乘于metavalue,倍数值最小为1
     *
     * @param value     double 待处理的数字
     * @param metavalue double 数字
    ...
    /**
     * Description: 该方法处理的过程如下: 1.将指定位数上的小数进行round 2.取得到的数字是metavalue的倍数值,并进行进位round 3.进位后的倍数值乘于metavalue,倍数值最小为1
     *
     * @param value     double 待处理的数字
     * @param digits    int 保留小数的位数
     * @param metavalue double 数字
     ...
     *
     * @param amount double 待truncate的数字
     ...
     * Description: 默认保留两位小数的截取方法
     *
     * @param amount BigDecimal 待truncate的数字
     ...
     * Description: 默认保留两位小数的截取方法
     *
     * @param amount String 待truncate的数字
     * @return BigDecimal truncate后的数字
     */
     ...
     *
     * @param doubleArray double[] double数字数组
     * @return double 最小的double数字
     */
     ...
     *
     * @param floatArray float[] float数字数组
     ...
     *
     * @param value  BigDecimal 待格式化的BigDecimal
     ...
     *
     * @param value  double 待格式化的double
     * @param format String 指定格式
     * @return String 格式化后的数字
     * @throws Exception
     */
    ...
    /**
     * 保留指定小数位数的四舍五入方法(返回string)
     *
     * @param amount
     * @param digits
     * @return
     */
    public static String round2str(double amount, int digits) {
        double result = round(amount, digits);
        String sFormat = "0";
        if (digits > 0) {
            sFormat = sFormat + "." + StringUtil.fillAfterZero("", digits);
        }
    ...
    public static String round2str(float amount) {
        return round2str(amount, DIGIT_DEFAULT_NUM);
    }

    public static String round2str(double amount) {
        return round2str(amount, DIGIT_DEFAULT_NUM);
    }

    public static String round2str(BigDecimal amount) {
        return round2str(amount, DIGIT_DEFAULT_NUM);
    }

    public static String round2str(BigDecimal amount, int digits) {
        BigDecimal result = round(amount, digits);
        LogUtil.debug(NumberUtils.class.toString(), "round bigdecimal result is: " + result);
        String sFormat = "0";
        if (digits > 0) {
            sFormat = sFormat + "." + StringUtil.fillAfterZero("", digits);
        }
